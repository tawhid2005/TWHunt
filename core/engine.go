...
func NewEngine(sources []Source, silent bool) *Engine {
	return &Engine{Sources: sources, Silent: silent}
}
...
func (e *Engine) Run(domain string) []string {
	...
	if !e.Silent {
		...
		// Just close the channel if silent so we don't leak it
		go func() { stopAnim <- true }()
	}
